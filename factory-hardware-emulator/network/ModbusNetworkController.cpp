...
void oldportal::fhe::network::ModbusNetworkController::run(oldportal::fhe::network::ModbusNetworkController* const  controller)
{//BEGIN_8e94675d1917f794cd825b81ee5da0b1
    assert(controller);
    assert(controller->_modbus_ctx);
    assert(controller->_query);
    ...
    controller->_run_thread_cycle_flag = true;
    while (controller->_run_thread_cycle_flag)
    {
        ...
        // bytes received:
        const int received_length = modbus_receive(controller->_modbus_ctx, controller->_query);
        ...
        if (received_length > 0) {
            ...
            const int header_length = modbus_get_header_length(controller->_modbus_ctx);
            // slave device address
            const uint8_t slave_address = controller->_query[header_length - 1];
            // Modbus function
            const uint8_t function = controller->_query[header_length];
            // Starting Address
            ...
            std::shared_ptr<oldportal::fhe::EmulatorApplication> application = controller->_application.lock();
            assert(application);
            ...
            for (auto device : application->_devices)
            {
                ...
                if (slave_address == MODBUS_BROADCAST_ADDRESS || slave_address == device->_modbus_address)
                {
                    ...
                    device->process_request(controller->_query, received_length, slave_address, function, controller->_modbus_ctx);
...
void oldportal::fhe::network::ModbusNetworkController::start()
{//BEGIN_370686dbbe5f52b9885670e71f68075e
    _thread = std::make_shared<std::thread>(oldportal::fhe::network::ModbusNetworkController::run, this);
}//END_370686dbbe5f52b9885670e71f68075e
